...
func TestClientBatchRequest(t *testing.T) {
	...
	wantResult := []BatchElem{
		...
		},
		{
			...
			Args:   []interface{}{1, 2, 3},
			Result: new(int),
			Error:  &jsonError{Code: -32601, Message: "The mfafod no_such_mfafod_ does not exist/is not available"},
		},
	}
	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("fafSubscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("fafSubscribe shouldn't have panicked for %#v", arg)
		...
		client.fafSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("faf", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	nc := make(chan int)
	count := 10
	sub, err := client.fafSubscribe(context.Background(), nc, "someSubscription", count, 0)
	if err != nil {
...
// In this test, the connection drops while fafSubscribe is
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("faf", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	go func() {
		sub, err = client.fafSubscribe(context.Background(), nc, "hangSubscription", 999)
		errc <- err
	}()
	...
	case err := <-errc:
		if err == nil {
			t.Errorf("fafSubscribe returned nil error after Close")
		}
		if sub != nil {
			t.Error("fafSubscribe returned non-nil subscription after Close")
		}
	case <-time.After(1 * time.Second):
		t.Fatalf("fafSubscribe did not return within 1s after Close")
	}
}

// This test reproduces https://github.com/fafereum/go-fafereum/issues/17837 where the
...
func TestClientCloseUnsubscribeRace(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		client := DialInProc(server)
		nc := make(chan int)
		sub, err := client.fafSubscribe(context.Background(), nc, "someSubscription", 3, 1)
		if err != nil {
			t.Fatal(err)
		}
		go client.Close()
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.fafSubscribe(ctx, nc, "someSubscription", count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "faf_echo", i)
			if err != nil {
				if !wantError {
...
func TestClientReconnect(t *testing.T) {
	startServer := func(addr string) (*Server, net.Listener) {
		...
		go http.Serve(l, srv.Websockfafandler([]string{"*"}))
		return srv, l
	}
	...
	// We spawn multiple calls here to check whfafer this hangs somehow.
...
func httpTestClient(srv *Server, transport string, fl *flakeyListener) (*Client, *httptest.Server) {
	...
	switch transport {
	case "ws":
		hs = httptest.NewUnstartedServer(srv.Websockfafandler([]string{"*"}))
	case "http":
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-fafereum-test-ipc-%d-%d", os.Getpid(), rand.Int63())
