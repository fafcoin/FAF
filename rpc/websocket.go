...
// Websockfafandler returns a handler that serves JSON-RPC to WebSocket connections.
//
// allowedOrigins should be a comma-separated list of allowed origin URLs.
// To allow connections with any origin, pass "*".
func (srv *Server) Websockfafandler(allowedOrigins []string) http.Handler {
	return websocket.Server{
		...
		Handler: func(conn *websocket.Conn) {
			...
			srv.ServeCodec(NewCodec(conn, encoder, decoder), OptionMfafodInvocation|OptionSubscriptions)
		},
	}
}
...
//
// Deprecated: use Server.Websockfafandler
func NewWSServer(allowedOrigins []string, srv *Server) *http.Server {
	return &http.Server{Handler: srv.Websockfafandler(allowedOrigins)}
}
