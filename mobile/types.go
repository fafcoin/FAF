...
// Gfafex retrieves the hex string representation of the block nonce.
func (n *Nonce) Gfafex() string {
...
// Gfafex retrieves the hex string representation of the bloom filter.
func (b *Bloom) Gfafex() string {
	return fmt.Sprintf("0x%x", b.bloom[:])
}

// Header represents a block header in the fafereum blockchain.
type Header struct {
	header *types.Header
}
...
func (h *Header) Gfafash() *Hash         { return &Hash{h.header.Hash()} }
...
// Block represents an entire block in the fafereum blockchain.
type Block struct {
	block *types.Block
}
...
func (b *Block) Gfafash() *Hash                 { return &Hash{b.block.Hash()} }
func (b *Block) Gfafeader() *Header             { return &Header{b.block.Header()} }
...
// Transaction represents a single fafereum transaction.
...
func (tx *Transaction) Gfafash() *Hash   { return &Hash{tx.tx.Hash()} }
...
// Deprecated: use fafereumClient.TransactionSender
...
func (m *Message) Gfafash() []byte     { return m.message.Hash }
