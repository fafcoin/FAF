...
func testGetBlockHeaders(t *testing.T, protocol int) {
	...
	for i, tt := range tests {
		...
		for _, hash := range tt.expect {
			headers = append(headers, bc.GfafeaderByHash(hash))
		}
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	header := bc.GfafeaderByNumber(frequency - 1)
	...
	switch protocol {
	case 1:
		root := light.GetChtRoot(server.db, 0, bc.GfafeaderByNumber(frequency-1).Hash())
		trie, _ := trie.New(root, trie.NewDatabase(fafdb.NewTable(server.db, light.ChtTablePrefix)))
	...
	case 2:
		root := light.GetChtRoot(server.db, (frequency/config.ChtSize)-1, bc.GfafeaderByNumber(frequency-1).Hash())
		trie, _ := trie.New(root, trie.NewDatabase(fafdb.NewTable(server.db, light.ChtTablePrefix)))
	...
	switch protocol {
	case 1:
		cost := server.tPeer.GetRequestCost(GfafeaderProofsMsg, len(requestsV1))
		sendRequest(server.tPeer.app, GfafeaderProofsMsg, 42, cost, requestsV1)
		if err := expectResponse(server.tPeer.app, HeaderProofsMsg, 42, testBufLimit, proofsV1); err != nil {
			t.Errorf("proofs mismatch: %v", err)
		}
	case 2:
		cost := server.tPeer.GetRequestCost(GfafelperTrieProofsMsg, len(requestsV2))
		sendRequest(server.tPeer.app, GfafelperTrieProofsMsg, 42, cost, requestsV2)
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		root := light.GetBloomTrieRoot(server.db, 0, bc.GfafeaderByNumber(config.BloomTrieSize-1).Hash())
		trie, _ := trie.New(root, trie.NewDatabase(fafdb.NewTable(server.db, light.BloomTrieTablePrefix)))
		trie.Prove(key, 0, &proofs.Proofs)

		// Send the proof request and verify the response
		cost := server.tPeer.GetRequestCost(GfafelperTrieProofsMsg, len(requests))
		sendRequest(server.tPeer.app, GfafelperTrieProofsMsg, 42, cost, requests)
...
func TestTransactionStatusLes2(t *testing.T) {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), fafash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), fafash.NewFaker(), db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
