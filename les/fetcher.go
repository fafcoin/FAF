...
func (f *lightFetcher) checkAnnouncedHeaders(fp *fetcherPeerInfo, headers []*types.Header, tds []*big.Int) bool {
	...
	for i := len(headers) - 1; ; i-- {
		if i < 0 {
			...
			header = f.chain.Gfafeader(hash, number)
...
func (f *lightFetcher) checkSyncedHeaders(p *peer) {
	...
	if n == nil {
		...
		header := f.chain.Gfafeader(n.hash, n.number)
...
// adds it to the end of a linked list togfafer with the time it has been confirmed. Then checks which peers have
