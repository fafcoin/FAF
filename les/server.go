...
func NewLesServer(faf *faf.fafereum, config *faf.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(faf.BlockChain().Config(), light.DefaultServerIndexerConfig, false, config.NetworkId, faf.EventMux(), faf.Engine(), newPeerSet(), faf.BlockChain(), faf.TxPool(), faf.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(faf.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		lesCommons: lesCommons{
			...
			chtIndexer:       light.NewChtIndexer(faf.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(faf.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		quitSync:  quitSync,
		lesTopics: lesTopics,
	...
	srv.chtIndexer.Start(faf.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(faf.ChainDb())
	return srv, nil
}
...
func newCostStats(db fafdb.Database) *requestCostStats {
