...
const (
	...
	fafVersion = 63 // equivalent faf version for the downloader
...
type BlockChain interface {
	...
	Gfafeader(hash common.Hash, number uint64) *types.Header
	GfafeaderByHash(hash common.Hash) *types.Header
...
// NewProtocolManager returns a new fafereum sub protocol manager. The fafereum sub protocol manages peers capable
// with the fafereum network.
func NewProtocolManager(chainConfig *params.ChainConfig, indexerConfig *light.IndexerConfig, lightSync bool, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb fafdb.Database, odr *LesOdr, txrelay *LesTxRelay, serverPool *serverPool, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light fafereum protocol")
	...
	log.Info("Light fafereum protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light fafereum peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light fafereum handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light fafereum peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light fafereum message handling failed", "err", err)
			return err
		}
	}
}
...
var (
	reqList   = []uint64{GetBlockHeadersMsg, GetBlockBodiesMsg, GetCodeMsg, GetReceiptsMsg, GetProofsV1Msg, SendTxMsg, SendTxV2Msg, GetTxStatusMsg, GfafeaderProofsMsg, GetProofsV2Msg, GfafelperTrieProofsMsg}
	reqListV1 = []uint64{GetBlockHeadersMsg, GetBlockBodiesMsg, GetCodeMsg, GetReceiptsMsg, GetProofsV1Msg, SendTxMsg, GfafeaderProofsMsg}
	reqListV2 = []uint64{GetBlockHeadersMsg, GetBlockBodiesMsg, GetCodeMsg, GetReceiptsMsg, SendTxV2Msg, GetTxStatusMsg, GetProofsV2Msg, GfafelperTrieProofsMsg}
)
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light fafereum message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case GetBlockHeadersMsg:
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit {
			...
			if hashMode {
				if first {
					first = false
					origin = pm.blockchain.GfafeaderByHash(query.Origin.Hash)
					if origin != nil {
						query.Origin.Number = origin.Number.Uint64()
					}
				} else {
					origin = pm.blockchain.Gfafeader(query.Origin.Hash, query.Origin.Number)
				}
			} else {
				origin = pm.blockchain.GfafeaderByNumber(query.Origin.Number)
			}
			if origin == nil {
				break
			}
			...
			case hashMode && !query.Reverse:
				...
				if next <= current {
					...
					if header := pm.blockchain.GfafeaderByNumber(next); header != nil {
	...
	case GetReceiptsMsg:
		...
		for _, hash := range req.Hashes {
			...
			if results == nil {
				if header := pm.blockchain.GfafeaderByHash(hash); header == nil || header.ReceiptHash != types.EmptyRootHash {
					continue
				}
			}
	...
	case GfafeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(fafdb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
			if header := pm.blockchain.GfafeaderByNumber(req.BlockNum); header != nil {
	...
	case GfafelperTrieProofsMsg:
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				var prefix string
				if root, prefix = pm.gfafelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(fafdb.NewTable(pm.chainDb, prefix)))
				}
			}
			if req.AuxReq == auxRoot {
				...
				if req.AuxReq != 0 {
					data := pm.gfafelperTrieAuxData(req)
					auxData = append(auxData, data)
					auxBytes += len(data)
				}
			}
...
// gfafelperTrie returns the post-processed trie root for the given trie ID and section index
func (pm *ProtocolManager) gfafelperTrie(id uint, idx uint64) (common.Hash, string) {
	switch id {
	case htCanonical:
...
// gfafelperTrieAuxData returns requested auxiliary data for the given HelperTrie request
func (pm *ProtocolManager) gfafelperTrieAuxData(req HelperTrieReq) []byte {
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, fafVersion, pc)
}
