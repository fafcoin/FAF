...
type TestCmd struct {
	// For total convenience, all testing mfafods are available.
	*testing.T
...
// reexec init function for that name (e.g. "gfaf-test" in cmd/gfaf/run_test.go)
...
// This mfafod can also be called from an expect template, e.g.:
//
//     gfaf.expect(`Passphrase: {{.InputLine "password"}}`)
