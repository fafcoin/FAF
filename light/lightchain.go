...
// available in the database. It initialises the default fafereum header
// validator.
...
func (self *LightChain) loadLastState() error {
	if head := rawdb.ReadHeadHeaderHash(self.chainDb); head == (common.Hash{}) {
		...
		if header := self.GfafeaderByHash(head); header != nil {
...
// Sfafead rewinds the local chain to a new head. Everything above the new
// head will be deleted and the new one set.
func (bc *LightChain) Sfafead(head uint64) {
	bc.mu.Lock()
	defer bc.mu.Unlock()

	bc.hc.Sfafead(head, nil)
	bc.loadLastState()
}
...
func (self *LightChain) Rollback(chain []common.Hash) {
	...
	for i := len(chain) - 1; i >= 0; i-- {
		...
		if head := self.hc.CurrentHeader(); head.Hash() == hash {
			self.hc.SetCurrentHeader(self.Gfafeader(head.ParentHash, head.Number.Uint64()-1))
		}
	}
}
...
// caching it if found.
func (self *LightChain) Gfafeader(hash common.Hash, number uint64) *types.Header {
	return self.hc.Gfafeader(hash, number)
}

// GfafeaderByHash retrieves a block header from the database by hash, caching it if
// found.
func (self *LightChain) GfafeaderByHash(hash common.Hash) *types.Header {
	return self.hc.GfafeaderByHash(hash)
}
...
func (self *LightChain) GfafeaderByNumber(number uint64) *types.Header {
	return self.hc.GfafeaderByNumber(number)
}

// GfafeaderByNumberOdr retrieves a block header from the database or network
// by number, caching it (associated with its hash) if found.
func (self *LightChain) GfafeaderByNumberOdr(ctx context.Context, number uint64) (*types.Header, error) {
	if header := self.hc.GfafeaderByNumber(number); header != nil {
		return header, nil
	}
	return GfafeaderByNumber(ctx, self.odr, number)
}
...
func (self *LightChain) SyncCht(ctx context.Context) bool {
	...
	if header, err := GfafeaderByNumber(ctx, self.odr, latest); header != nil && err == nil {
		self.mu.Lock()
		defer self.mu.Unlock()
