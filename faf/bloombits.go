...
func (faf *fafereum) startBloomHandlers(sectionSize uint64) {
			...
			for {
				...
				case request := <-faf.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(faf.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(faf.chainDb, task.Bit, section, head); err == nil {
...
// for the fafereum header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      fafdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db fafdb.Database, size, confirms uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := fafdb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
