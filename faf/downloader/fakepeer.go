...
func NewFakePeer(id string, db fafdb.Database, hc *core.HeaderChain, dl *Downloader) *FakePeer {
...
func (p *FakePeer) RequestHeadersByHash(hash common.Hash, amount int, skip int, reverse bool) error {
	...
	for !unknown && len(headers) < amount {
		origin := p.hc.GfafeaderByHash(hash)
		if origin == nil {
			break
		}
		...
		if reverse {
			for i := 0; i <= skip; i++ {
				if header := p.hc.Gfafeader(hash, number); header != nil {
			...
			if header := p.hc.GfafeaderByNumber(next); header != nil {
...
func (p *FakePeer) RequestHeadersByNumber(number uint64, amount int, skip int, reverse bool) error {
	...
	for !unknown && len(headers) < amount {
		origin := p.hc.GfafeaderByNumber(number)
		if origin == nil {
			break
		}
		if reverse {
