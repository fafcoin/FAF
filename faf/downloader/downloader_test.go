...
type downloadTester struct {
	...
	stateDb fafdb.Database // Database used by the tester for syncing from peers
	peerDb  fafdb.Database // Database of the peers containing all data
...
func newTester() *downloadTester {
	...
	tester.stateDb = fafdb.NewMemDatabase()
...
func (dl *downloadTester) HasHeader(hash common.Hash, number uint64) bool {
	return dl.GfafeaderByHash(hash) != nil
}
...
// Gfafeader retrieves a header from the testers canonical chain.
func (dl *downloadTester) GfafeaderByHash(hash common.Hash) *types.Header {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
// RequestBodies constructs a getBlockBodies mfafod associated with a particular
...
// RequestReceipts constructs a getReceipts mfafod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
// RequestNodeData constructs a getNodeData mfafod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
func assertOwnChain(t *testing.T, tester *downloadTester, length int) {
	// Mark this mfafod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", fafereum.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", fafereum.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", fafereum.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", fafereum.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want fafereum.SyncProgress) {
	// Mark this mfafod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", fafereum.SyncProgress{
