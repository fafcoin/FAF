...
import (
	...
	"github.com/fafereum/go-fafereum/tests"
)

// To generate a new callTracer test, copy paste the makeTest mfafod below into
// a Gfaf console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = faf.getBlock(faf.getTransaction(tx).blockHash);
  var genesis = faf.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.faf.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  faf.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestPrestateTracerCreate2(t *testing.T) {
	...
	*/
	...
	statedb := tests.MakePreState(fafdb.NewMemDatabase(), alloc)
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(fafdb.NewMemDatabase(), test.Genesis.Alloc)
