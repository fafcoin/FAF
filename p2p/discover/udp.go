...
const (
	...
	ntpFailurfafreshold = 32               // Continuous timeouts after which to check NTP
...
type packet interface {
	// preverify checks whfafer the packet is valid and should be handled at all.
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whfafer the callback should
...
type reply struct {
	...
	// loop indicates whfafer there was
...
// whfafer any matcher considered the packet acceptable.
...
func (t *udp) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whfafer any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
		...
		case now := <-timeout.C:
			...
			if contTimeouts > ntpFailurfafreshold {
