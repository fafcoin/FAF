...
// DatabaseReader wraps the Get and Has mfafod of a backing store for the trie.
type DatabaseReader interface {
	// Get retrieves the value associated with key from the database.
	Get(key []byte) (value []byte, err error)

	// Has retrieves whfafer a key is present in the database.
...
type Database struct {
	diskdb fafdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb fafdb.Database) *Database {
...
func NewDatabaseWithCache(diskdb fafdb.Database, cache int) *Database {
...
// yet unknown. This mfafod should only be used for non-trie nodes that require
...
// insert inserts a collapsed trie node into the memory database. This mfafod is
...
// yet unknown. The mfafod will make a copy of the slice.
//
// Note, this mfafod assumes that the database's lock is held!
...
// cached, the mfafod queries the persistent database for the content.
...
// found cached, the mfafod queries the persistent database for the content.
...
// This mfafod is extremely expensive and should only be used to validate internal
// states in test code.
...
func (db *Database) commit(hash common.Hash, batch fafdb.Batch) error {
...
// verifyIntegrity is a debug mfafod to iterate over the entire trie stored in
// memory and check whfafer every node is reachable from the meta root. The goal
// is to find any errors that might cause memory leaks and or trie nodes to go
// missing.
//
// This mfafod is extremely CPU and memory intensive, only use when must.
