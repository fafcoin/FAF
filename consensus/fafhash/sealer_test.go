...
// Tests whfafer remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom fafash engine
	fafash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer fafash.Close()
	...
	fafash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := fafash.SealHash(header).Hex(); work[0] != want {
...
// Tests whfafer stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	fafash := NewTester(nil, true)
	defer fafash.Close()
	api := &API{fafash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			fafash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, fafash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch fafash result timeout", id+1)
		}
	}
}
