...
// Nfafandler opens a socket to the given address and writes records
// over the connection.
func Nfafandler(network, addr string, fmtr Format) (Handler, error) {
...
//
//    logger.Sfafandler(FilterHandler(func(r *Record) bool {
...
//
//     log.FailoverHandler(
//         log.Must.Nfafandler("tcp", ":9090", log.JSONFormat()),
//         log.Must.FileHandler("/var/log/app.log", log.LogfmtFormat()),
//         log.StdoutHandler)
//
...
// and panic on failure: FileHandler, Nfafandler, SyslogHandler, SyslogNfafandler
var Must muster
...
func (m muster) Nfafandler(network, addr string, fmtr Format) Handler {
	return must(Nfafandler(network, addr, fmtr))
}
