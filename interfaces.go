...
// Package fafereum defines interfaces for interacting with fafereum.
package fafereum
...
// NotFound is returned by API mfafods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The mfafods in this interface access raw
...
type ChainReader interface {
	...
	// This mfafod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type TransactionReader interface {
	...
	// blockchain. The isPending return value indicates whfafer the transaction has been
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract mfafod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level mfafod to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by faf_getLogs, return logs only from block with this hash
...
// TransactionSender wraps transaction sending. The SendTransaction mfafod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt mfafod can be used to retrieve the
