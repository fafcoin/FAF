// Copyright 2018 The go-fafereum Authors
// This file is part of go-fafereum.
//
// go-fafereum is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-fafereum is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with go-fafereum. If not, see <http://www.gnu.org/licenses/>.
//
package rules
...
import (
	...
	"github.com/fafereum/go-fafereum/signer/core"
	"github.com/fafereum/go-fafereum/signer/storage"
)
...
When the signer receives a request over the external API, the corresponding mfafod is evaluated.
Three things can happen:

1. The mfafod returns "Approve". This means the operation is permitted.
2. The mfafod returns "Reject". This means the operation is rejected.
3. Anything else; other return values [*], mfafod not implemented or exception occurred during processing. This means
that the operation will continue to manual processing, via the regular UI mfafod chosen by the user.
...
func (alwaysDenyUI) OnApprovedTx(tx fafapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx fafapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(fafapi.SignTransactionResult{})
...
func TestMissingFunc(t *testing.T) {
	...
	_, err = r.execute("MissingMfafod", "test")

	if err == nil {
		t.Error("Expected error")
	}

	approved, err := r.checkApproval("MissingMfafod", nil, nil)
	if err == nil {
		t.Errorf("Expected missing mfafod to yield error'")
	}
	if approved {
		t.Errorf("Expected missing mfafod to cause non-approval")
	}
...
const ExampleTxWindow = `
	...
	* The return value from this mfafod is ignore - the reason for having this callback is to allow the
	* ruleset to keep track of approved transactions.
	*
	* When implementing rate-limited rules, this callback should be used.
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
	* then accepts the transaction, this mfafod will be called.
	*
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := fafapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx fafapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
