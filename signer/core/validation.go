...
func testSelector(selector string, data []byte) (*decodedCallData, error) {
	...
	abiData, err := MfafodSelectorToAbi(selector)
	if err != nil {
		return nil, err
	}
...
// validateCallData checks if the ABI-data + mfafodselector (if given) can be parsed and seems to match
func (v *Validator) validateCallData(msgs *ValidationMessages, data []byte, mfafodSelector *string) {
	if len(data) == 0 {
		return
	}
	if len(data) < 4 {
	...
	if mfafodSelector != nil {
		info, err = testSelector(*mfafodSelector, data)
		if err != nil {
			...
			v.db.AddSignature(*mfafodSelector, data[:4])
		}
		return
	}
	// Check the db
	selector, err := v.db.LookupMfafodSelector(data[:4])
	if err != nil {
...
func (v *Validator) validate(msgs *ValidationMessages, txargs *SendTxArgs, mfafodSelector *string) error {
	...
	if txargs.To == nil {
		...
		// e.g. https://github.com/fafereum/go-fafereum/issues/16106
		if len(data) == 0 {
			if txargs.Value.ToInt().Cmp(big.NewInt(0)) > 0 {
				// Sending fafer into black hole
		...
		// mfafodSelector should be nil for contract creation
		if mfafodSelector != nil {
			msgs.warn("Tx will create contract, but mfafod selector supplied; indicating intent to call a mfafod.")
		}
		...
		// Validate calldata
		v.validateCallData(msgs, data, mfafodSelector)
	}
	return nil
}
...
func (v *Validator) ValidateTransaction(txArgs *SendTxArgs, mfafodSelector *string) (*ValidationMessages, error) {
	msgs := &ValidationMessages{}
	return msgs, v.validate(msgs, txArgs, mfafodSelector)
}
