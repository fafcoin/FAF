...
func (ui *HeadlessUI) OnApprovedTx(tx fafapi.SignTransactionResult) {
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "faf-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *fafapi.SignTransactionResult
		err       error
	)
	...
	control <- "Y"
	control <- "wrongpassword"
	res, err = api.SignTransaction(context.Background(), tx, &mfafodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &mfafodSig)
	if err != nil {
		t.Fatal(err)
	}
