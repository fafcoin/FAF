...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, mfafodSelector *string) (*fafapi.SignTransactionResult, error)
...
// SignerUI specifies what mfafod a UI needs to implement to be able to be used as a UI for the signer
type SignerUI interface {
	...
	// This mfafod can be used by a UI to keep track of e.g. how much has been sent to a particular recipient.
	OnApprovedTx(tx fafapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, mfafodSelector *string) (*fafapi.SignTransactionResult, error) {
	var (
		err    error
		result SignTxResponse
	)
	msgs, err := api.validator.ValidateTransaction(&args, mfafodSelector)
	if err != nil {
		return nil, err
	}
	...
	response := fafapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
// OBS! This mfafod is removed from the public API. It should not be exposed on the external API
