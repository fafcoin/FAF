...
func verify(t *testing.T, jsondata, calldata string, exp []interface{}) {
	...
	data, err := mfafod.Inputs.UnpackValues(argdata)
	if err != nil {
		t.Fatal(err)
	}
	if len(data) != len(exp) {
...
func TestNewUnpacker(t *testing.T) {
	...
	testcases := []unpackTest{
		...
		}, { // https://github.com/fafereum/wiki/wiki/fafereum-Contract-ABI#examples
...
[
	...
	for i, hexdata := range []string{
		...
		// From https://github.com/fafereum/wiki/wiki/fafereum-Contract-ABI
...
func TestSelectorUnmarshalling(t *testing.T) {
	...
	for id, selector := range db.db {
		...
		abistring, err = MfafodSelectorToAbi(selector)
		if err != nil {
			t.Error(err)
			return
		}
		...
		m, err := abistruct.MfafodById(common.Hex2Bytes(id[2:]))
		if err != nil {
			t.Error(err)
			return
		}
...
func TestCustomABI(t *testing.T) {
	...
	_, err = abidb.LookupMfafodSelector(calldata)
	if err == nil {
	...
	_, err = abidb2.LookupMfafodSelector(calldata)
	if err != nil {
