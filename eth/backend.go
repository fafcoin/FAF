...
func New(stack *node.Node, config *Config) (*Ethereum, error) {
	...
	eth.netRPCService = fafapi.NewPublicNetAPI(eth.p2pServer, eth.NetVersion())
...
func CreateConsensusEngine(stack *node.Node, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db fafdb.Database) consensus.Engine {
...
func (s *Ethereum) ChainDb() fafdb.Database  { return s.chainDb }
