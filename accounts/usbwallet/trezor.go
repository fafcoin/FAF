...
import (
	...
	"github.com/fafereum/go-fafereum/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whfafer the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whfafer the Trezor is opened, closed
// or whfafer the fafereum app was not started on it.
...
// fafereum address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.fafereumAddress)
	if _, err := w.trezorExchange(&trezor.fafereumGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.fafereumTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.fafereumTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mfafod will also return the index of the destination object used.
