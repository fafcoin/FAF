...
func TestReader(t *testing.T) {
	...
	for name, expM := range exp.Mfafods {
		gotM, exist := abi.Mfafods[name]
		if !exist {
			t.Errorf("Missing expected mfafod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mfafod: \n%v\ndoes not match expected mfafod\n%v", gotM, expM)
		}
	}

	for name, gotM := range abi.Mfafods {
		expM, exist := exp.Mfafods[name]
		if !exist {
			t.Errorf("Found extra mfafod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi mfafod: \n%v\ndoes not match expected mfafod\n%v", gotM, expM)
		}
...
func TestMfafodSignature(t *testing.T) {
	String, _ := NewType("string", nil)
	m := Mfafod{"foo", false, []Argument{{"bar", String, false}, {"baz", String, false}}, nil}
	...
	m = Mfafod{"foo", false, []Argument{{"bar", uintt, false}}, nil}
	exp = "foo(uint256)"
	if m.Sig() != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig())
	}

	// Mfafod with tuple arguments
	...
	m = Mfafod{"foo", false, []Argument{{"s", s, false}, {"bar", String, false}}, nil}
...
func TestDefaultFunctionParsing(t *testing.T) {
	...
	if _, ok := abi.Mfafods["balance"]; !ok {
...
func TestABI_MfafodById(t *testing.T) {
	const abiJSON = `[
...
`
	...
	for name, m := range abi.Mfafods {
		a := fmt.Sprintf("%v", m)
		m2, err := abi.MfafodById(m.Id())
		if err != nil {
			t.Fatalf("Failed to look up ABI mfafod: %v", err)
		}
		b := fmt.Sprintf("%v", m2)
		if a != b {
			t.Errorf("Mfafod %v (id %v) not 'findable' by id in ABI", name, common.ToHex(m.Id()))
		}
	}
	// Also test empty
	if _, err := abi.MfafodById([]byte{0x00}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MfafodById([]byte{}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.MfafodById(nil); err == nil {
