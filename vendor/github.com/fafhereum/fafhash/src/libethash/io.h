...
// Reference: https://github.com/fafereum/wiki/wiki/fafash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see fafash_io_prepare
enum fafash_io_rc {
	fafASH_IO_FAIL = 0,           ///< There has been an IO failure
	fafASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	fafASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	fafASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of fafash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * fafash_full_t
 */
#ifdef fafASH_PRINT_CRITICAL_OUTPUT
#define fafASH_CRITICAL(...)							\
	do													\
	{													\
		printf("fafASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for fafash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the fafash
 ...
 *                           https://github.com/fafereum/wiki/wiki/fafash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum fafash_io_rc
 */
 ...
 */
FILE* fafash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* fafash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool fafash_mkdir(char const* dirname);
 ...
 */
bool fafash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* fafash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/fafereum/wiki/wiki/fafash-DAG
 *
 ...
 */
bool fafash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool fafash_io_mutable_name(
	uint32_t revision,
