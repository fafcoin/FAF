...
// To change the pipeline, create the BlobURL and then call its WithPipeline mfafod passing in the
...
// To change the pipeline, create the AppendBlobURL and then call its WithPipeline mfafod passing in the
// desired pipeline object. Or, call this package's NewAppendBlobURL instead of calling this object's
// NewAppendBlobURL mfafod.
...
// To change the pipeline, create the BlockBlobURL and then call its WithPipeline mfafod passing in the
// desired pipeline object. Or, call this package's NewBlockBlobURL instead of calling this object's
// NewBlockBlobURL mfafod.
...
// To change the pipeline, create the PageBlobURL and then call its WithPipeline mfafod passing in the
// desired pipeline object. Or, call this package's NewPageBlobURL instead of calling this object's
// NewPageBlobURL mfafod.
...
func (c ContainerURL) GetProperties(ctx context.Context, ac LeaseAccessConditions) (*ContainerGetPropertiesResponse, error) {
	...
	// This allows us to not expose a GetProperties mfafod at all simplifying the API.
...
// GetAccessPolicy returns the container's access policy. The access policy indicates whfafer container's blobs may be accessed publicly.
...
// Initialize an instance of this type and then call its String mfafod to set AccessPolicy's Permission field.
...
// Call this mfafod to set AccessPolicy's Permission field.
...
// SetAccessPolicy sets the container's permissions. The access policy indicates whfafer blobs in a container may be accessed publicly.
