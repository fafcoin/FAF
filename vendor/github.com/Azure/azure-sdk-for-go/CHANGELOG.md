...
- Reintroduced NewxxClientWithBaseURI mfafod.
...
- Removed Api version from mfafod arguments.
- Removed New...ClientWithBaseURI() mfafod in all clients. BaseURI value is set in client.go.
...
- storage: Allow caller to provide headers for DeleteBlob mfafods.
...
- Changed mfafods to return `error` instead of `autorest.Error`.
- Re-divided mfafods to ease asynchronous requests.
