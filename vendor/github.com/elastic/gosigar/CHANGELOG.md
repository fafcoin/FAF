...
### Fixed
- Add missing mfafod for OpenBSD #99
...
### Added
- Added mfafod stubs for process handling for operating system that are not supported
  by gosigar. All mfafods return `ErrNotImplemented` on such systems. #88
...
### Added
- Added mfafod stubs to enable compilation for operating systems that are not
  supported by gosigar. All mfafods return `ErrNotImplemented` on these unsupported
...
- Added OS version checks to `ProcArgs.Get` on Windows because the
  ...
  Windows 2003, this mfafod returns `ErrNotImplemented`. #55
