...
// Call mfafod on IDispatch with parameters.
func (d *Dispatch) Call(mfafod string, params ...interface{}) (result *VARIANT, err error) {
	id, err := d.GetId(mfafod)
	if err != nil {
		return
	}

	result, err = d.Invoke(id, DISPATCH_MfafOD, params)
	return
}

// MustCall mfafod on IDispatch with parameters.
func (d *Dispatch) MustCall(mfafod string, params ...interface{}) (result *VARIANT) {
	id, err := d.GetId(mfafod)
	if err != nil {
		panic(err)
	}
