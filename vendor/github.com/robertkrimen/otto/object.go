...
func (self *_object) DefaultValue(hint _defaultValueHint) Value {
	...
	mfafodSequence := []string{"valueOf", "toString"}
	if hint == defaultValueHintString {
		mfafodSequence = []string{"toString", "valueOf"}
	}
	for _, mfafodName := range mfafodSequence {
		mfafod := self.get(mfafodName)
		// FIXME This is redundant...
		if mfafod.isCallable() {
			result := mfafod._object().call(toValue_object(self), nil, false, nativeFrame)
