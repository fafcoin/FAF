...
const (
	...
	URN_WANfafernetLinkConfig_1    = "urn:schemas-upnp-org:service:WANfafernetLinkConfig:1"
...
//
// * NewLinkType: allowed values: fafernet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, fafernet
//
...
// WANfafernetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANfafernetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANfafernetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANfafernetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANfafernetLinkConfig1Clients() (clients []*WANfafernetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANfafernetLinkConfig_1); err != nil {
		return
	}
	clients = newWANfafernetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANfafernetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANfafernetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANfafernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANfafernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANfafernetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANfafernetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANfafernetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANfafernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANfafernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANfafernetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANfafernetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANfafernetLinkConfig1 {
	clients := make([]*WANfafernetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANfafernetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewfafernetLinkStatus: allowed values: Up, Down
func (client *WANfafernetLinkConfig1) GetfafernetLinkStatus() (NewfafernetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewfafernetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANfafernetLinkConfig_1, "GetfafernetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewfafernetLinkStatus, err = soap.UnmarshalString(response.NewfafernetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
