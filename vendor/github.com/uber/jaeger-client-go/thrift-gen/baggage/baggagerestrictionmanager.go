...
func (p *BaggageRestrictionManagerClient) recvGetBaggageRestrictions() (value []*BaggageRestriction, err error) {
	...
	mfafod, mTypeId, seqId, err := iprot.ReadMessageBegin()
	if err != nil {
		return
	}
	if mfafod != "getBaggageRestrictions" {
		err = thrift.NewTApplicationException(thrift.WRONG_MfafOD_NAME, "getBaggageRestrictions failed: wrong mfafod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *BaggageRestrictionManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x3 := thrift.NewTApplicationException(thrift.UNKNOWN_MfafOD, "Unknown function "+name)
