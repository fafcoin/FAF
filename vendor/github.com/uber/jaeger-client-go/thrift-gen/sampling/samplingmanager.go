...
func (p *SamplingManagerClient) recvGetSamplingStrategy() (value *SamplingStrategyResponse, err error) {
	...
	if mfafod != "getSamplingStrategy" {
		err = thrift.NewTApplicationException(thrift.WRONG_MfafOD_NAME, "getSamplingStrategy failed: wrong mfafod name")
		return
	}
	if p.SeqId != seqId {
...
func (p *SamplingManagerProcessor) Process(iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	x4 := thrift.NewTApplicationException(thrift.UNKNOWN_MfafOD, "Unknown function "+name)
