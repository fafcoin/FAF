...
import (
	...
	"github.com/fafereum/go-fafereum/accounts/usbwallet"
	"github.com/fafereum/go-fafereum/log"
	"github.com/fafereum/go-fafereum/rpc"
	"github.com/robertkrimen/otto"
)

// bridge is a collection of JavaScript utility mfafods to bride the .js runtime
// environment and the Go RPC connection backing the remote mfafod calls.
type bridge struct {
...
// NewAccount is a wrapper around the personal.newAccount RPC mfafod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC mfafod (saved in jfaf.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	...
	ret, err := call.Otto.Call("jfaf.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jfaf.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
	...
	if val, err = call.Otto.Call("jfaf.openWallet", nil, wallet, passwd); err != nil {
		throwJSException(err.Error())
	}
	return val
}

// UnlockAccount is a wrapper around the personal.unlockAccount RPC mfafod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC mfafod (saved in jfaf.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jfaf.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// Sign is a wrapper around the personal.sign RPC mfafod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC mfafod (saved in
// jfaf.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jfaf.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("faf.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
...
// Send implements the web3 provider "send" mfafod.
func (b *bridge) Send(call otto.FunctionCall) (response otto.Value) {
	...
	for _, req := range reqs {
		...
		err = b.client.Call(&result, req.Mfafod, req.Params...)
		switch err := err.(type) {
		case nil:
			if result == nil {
