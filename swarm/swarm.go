...
import (
	...
	"github.com/fafereum/go-fafereum/contracts/chequebook"
	"github.com/fafereum/go-fafereum/contracts/ens"
	...
	httpapi "github.com/fafereum/go-fafereum/swarm/api/http"
	"github.com/fafereum/go-fafereum/swarm/fuse"
	"github.com/fafereum/go-fafereum/swarm/log"
	"github.com/fafereum/go-fafereum/swarm/network"
	"github.com/fafereum/go-fafereum/swarm/network/stream"
	"github.com/fafereum/go-fafereum/swarm/pss"
	...
	"github.com/fafereum/go-fafereum/swarm/swap"
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = fafclient.Dial(config.SwapAPI)
		if err != nil {
	...
	if pss.IsActiveHandshake {
		pss.SfafandshakeController(self.ps, pss.NewHandshakeParams())
	}
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	fafClient := fafclient.NewClient(client)
	...
	dns, err := ens.NewENS(transactOpts, ensRoot, fafClient)
	if err != nil {
		return nil, err
	}
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := fafclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
