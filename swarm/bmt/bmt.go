...
type node struct {
	isLeft      bool      // whfafer it is left side of the parent double segment
...
// mfafods needed to implement hash.Hash
...
// hash.Hash interface Sum mfafod appends the byte slice to the underlying
...
// mfafods needed to implement the SwarmHash and the io.Writer interfaces
...
// mfafods needed to implement the SwarmHash interface
...
type AsyncHasher struct {
	*Hasher            // extends the Hasher
	...
	double  bool       // whfafer to use double segments (call Hasher.writeSection)
	secsize int        // size of base section (size of hash or double)
	write   func(i int, section []byte, final bool)
}

// mfafods needed to implement AsyncWriter
...
// meta: metadata to hash togfafer with BMT root for the final digest
...
func (sw *AsyncHasher) Sum(b []byte, length int, meta []byte) (s []byte) {
	...
	// hash togfafer meta and BMT root hash using the pools
