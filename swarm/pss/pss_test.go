...
// matching of address hints; whfafer a message could be or is for the node
...
func TestAddressMatchProx(t *testing.T) {
	...
	// first the unit test on the mfafod that calculates possible receipient using prox
...
func newServices(allowRaw bool) adapters.Services {
	...
	return adapters.Services{
		pssProtocolName: func(ctx *adapters.ServiceContext) (node.Service, error) {
			...
			if useHandshake {
				SfafandshakeController(ps, NewHandshakeParams())
			}
