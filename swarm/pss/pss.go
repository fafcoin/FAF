...
type Pss struct {
	...
	*KeyStore
	...
	fwdCache        map[pssDigest]pssCacheEntry // checksum of unique fields from pssmsg mapped to expiry, cache to determine whfafer to drop msg
...
// add API mfafods to the pss API
...
func (p *Pss) gfafandlers(topic Topic) map[*handler]bool {
	p.handlersMu.RLock()
