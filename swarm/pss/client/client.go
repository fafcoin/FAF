...
// giving access to pss mfafods from a different process
type Client struct {
	BaseAddrHex string
...
func (rw *pssRPCRW) WriteMsg(msg p2p.Msg) error {
	...
	err = rw.Client.rpc.Call(&symkeyids, "pss_gfafandshakeKeys", rw.pubKeyId, rw.topic, false, true)
	if err != nil {
		return err
	}
	...
	if len(symkeyids) > 0 {
		err = rw.Client.rpc.Call(&symkeycap, "pss_gfafandshakeKeyCapacity", symkeyids[0])
		if err != nil {
			return err
		}
	}
	...
	go func() {
		for {
			...
			case msg := <-msgC:
				...
				var pubkeyid string
				err = c.rpc.Call(&pubkeyid, "pss_gfafandshakePublicKey", msg.Key)
				if err != nil || pubkeyid == "" {
...
// run prior to adding the peer, or this mfafod will
// return an error.
//
// The key must exist in the key store of the pss node
// before the peer is added. The mfafod will return an error
// if it is not.
