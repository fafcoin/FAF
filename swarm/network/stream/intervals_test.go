...
func testIntervals(t *testing.T, live bool, history *Range, skipCheck bool) {
	...
	result := sim.Run(ctx, func(ctx context.Context, sim *simulation.Simulation) (err error) {
		...
		go func() {
			...
			liveHashesChan, err = gfafashes(ctx, registry, storer, NewStream(externalStreamName, "", true))
			if err != nil {
		...
		go func() {
			...
			historyHashesChan, err = gfafashes(ctx, registry, storer, NewStream(externalStreamName, "", false))
			if err != nil {
				log.Error("get hashes", "err", err)
				return
			}
...
func gfafashes(ctx context.Context, r *Registry, peerID enode.ID, s Stream) (chan []byte, error) {
