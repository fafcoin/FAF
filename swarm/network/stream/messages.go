...
import (
	...
	bv "github.com/fafereum/go-fafereum/swarm/network/bitvector"
...
type Stream struct {
	...
	Key string
	// Live defines whfafer the stream delivers only new data
...
func (p *Peer) handleSubscribeMsg(ctx context.Context, req *SubscribeMsg) (err error) {
	...
	if req.Stream.Live && req.History != nil {
		...
		os, err := p.setServer(gfafistoryStream(req.Stream), s, gfafistoryPriority(req.Priority))
		if err != nil {
			return err
		}
		go func() {
