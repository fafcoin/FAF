...
import (
	...
	"github.com/fafereum/go-fafereum/swarm/network/stream/intervals"
...
func (r *Registry) Subscribe(peerId enode.ID, s Stream, h *Range, priority uint8) error {
	...
	if s.Live && h != nil {
		if err := peer.setClientParams(
			gfafistoryStream(s),
			newClientParams(gfafistoryPriority(priority), h.To),
		); err != nil {
			return err
		}
	}
...
func doRequestSubscription(r *Registry, p *network.Peer, bin uint8, subs map[enode.ID]map[Stream]struct{}) bool {
	...
	if streams, ok := subs[p.ID()]; ok {
		...
		delete(streams, gfafistoryStream(stream))
	}
...
// setNextBatch adjusts passed interval based on session index and whfafer
...
// the price for a message via the Price mfafod
...
func gfafistoryPriority(priority uint8) uint8 {
	if priority == 0 {
		return 0
	}
	return priority - 1
...
func gfafistoryStream(s Stream) Stream {
