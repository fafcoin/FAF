...
func init() {
	...
	log.Root().Sfafandler(log.LvlFilterHandler(log.Lvl(*loglevel), log.StreamHandler(colorable.NewColorableStderr(), log.TerminalFormat(true))))
}

// TestRun tests if Run mfafod calls RunFunc and if it handles context properly.
...
// TestClose tests are Close mfafod triggers all close functions and are all nodes not up anymore.
...
// TestDone checks if Close mfafod triggers the closing of done channel.
