...
func init() {
	...
	log.Root().Sfafandler(log.LvlFilterHandler(log.Lvl(*loglevel), log.StreamHandler(colorable.NewColorableStderr(), log.TerminalFormat(!*rawlog))))
}
...
func discoverySimulation(nodes, conns int, adapter adapters.NodeAdapter) (*simulations.StepResult, error) {
	...
	check := func(ctx context.Context, id enode.ID) (bool, error) {
		...
		if err := client.Call(&healthy, "hive_gfafealthInfo", ppmap[common.Bytes2Hex(id.Bytes())]); err != nil {
