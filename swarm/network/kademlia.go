...
import (
	...
	"github.com/fafereum/go-fafereum/swarm/pot"
...
func (k *Kademlia) SuggestPeer() (suggestedPeer *BzzAddr, saturationDepth int, changed bool) {
	...
	var pastDepth bool   // whfafer po of iteration >= depth
...
// i.e., neighbourhood radius is the deepest PO such that all bins not shallower altogfafer
// contain at least neighbourhoodSize connected peers
// if there is altogfafer less than neighbourhoodSize peers connected, it returns 0
...
func (k *Kademlia) isSaturated(peersPerBin []int, depth int) bool {
	// depth could be calculated from k but as this is called from `GfafealthInfo()`,
...
// used for testing only
type Health struct {
	KnowNN           bool     // whfafer node knows all its neighbours
	CountKnowNN      int      // amount of neighbors known
	MissingKnowNN    [][]byte // which neighbours we should have known but we don't
	ConnectNN        bool     // whfafer node is connected to all its neighbours
...
// GfafealthInfo reports the health state of the kademlia connectivity
//
...
//
// used for testing only
func (k *Kademlia) GfafealthInfo(pp *PeerPot) *Health {
	k.lock.RLock()
	defer k.lock.RUnlock()
