...
#
# The cluster contains a bootnode, a gfaf node and multiple swarm nodes, with
...
# static gfaf configuration
Gfaf_IP="192.168.33.3"
Gfaf_RPC_PORT="8545"
Gfaf_RPC_URL="http://${Gfaf_IP}:${Gfaf_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gfaf and
# swarm nodes
start_bootnode() {
...
# start_gfaf_node starts a gfaf node with --datadir pointing at <base-dir>/gfaf
# and a single, unlocked account with password "gfaf"
start_gfaf_node() {
  local dir="${base_dir}/gfaf"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gfaf account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Gfaf_IP}"
    --rpcport   "${Gfaf_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gfaf" "${Gfaf_IP}" "$(which gfaf)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vfaf pair
  local vfaf0="vfaf${name}0"
  local vfaf1="vfaf${name}1"
  ip link add name "${vfaf0}" type vfaf peer name "${vfaf1}"

  # add one end to the bridge
  ip link set dev "${vfaf0}" master "${BRIDGE_NAME}"
  ip link set dev "${vfaf0}" up

  # add the other end to the namespace, rename it faf0 and give it the ip
  ip link set dev "${vfaf1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vfaf1}" name "faf0"
  ip netns exec "${name}" ip link set dev "faf0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "faf0"
}
...
create_account() {
  ...
  gfaf --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
