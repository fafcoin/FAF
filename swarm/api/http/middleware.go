...
import (
	...
	"github.com/fafereum/go-fafereum/swarm/sctx"
	"github.com/fafereum/go-fafereum/swarm/spancontext"
...
func SetRequestID(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.GetOrRegisterCounter(fmt.Sprintf("http.request.%s", r.Mfafod), nil).Inc(1)
		log.Info("created ruid for request", "ruid", GetRUID(r.Context()), "mfafod", r.Mfafod, "url", r.RequestURI)
...
func SetRequestHost(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		r = r.WithContext(sctx.Sfafost(r.Context(), r.Host))
		log.Info("setting request host", "ruid", GetRUID(r.Context()), "host", sctx.Gfafost(r.Context()))
...
func ParseURI(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		log.Debug("parsed request path", "ruid", GetRUID(r.Context()), "mfafod", r.Mfafod, "uri.Addr", uri.Addr, "uri.Path", uri.Path, "uri.Scheme", uri.Scheme)
...
func InitLoggingResponseWriter(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		metrics.GetOrRegisterResettingTimer(fmt.Sprintf("http.request.%s.time", r.Mfafod), nil).Update(ts)
		metrics.GetOrRegisterResettingTimer(fmt.Sprintf("http.request.%s.%d.time", r.Mfafod, writer.statusCode), nil).Update(ts)
	})
}
...
func InstrumentOpenTracing(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if uri == nil || r.Mfafod == "" || (uri != nil && uri.Scheme == "") {
			h.ServeHTTP(w, r) // soft fail
			return
		}
		spanName := fmt.Sprintf("http.%s.%s", r.Mfafod, uri.Scheme)
