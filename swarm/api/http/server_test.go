...
import (
	...
	swarm "github.com/fafereum/go-fafereum/swarm/api/client"
	"github.com/fafereum/go-fafereum/swarm/storage"
	"github.com/fafereum/go-fafereum/swarm/storage/feed"
...
func init() {
	...
	flag.Parse()
	log.Root().Sfafandler(log.CallerFileHandler(log.LvlFilterHandler(log.Lvl(*loglevel), log.StreamHandler(os.Stderr, log.TerminalFormat(true)))))
}
...
func TestBzzWithFeed(t *testing.T) {
	...
	// put togfafer some data for our test:
	dataBytes := []byte(`
	//
...
// Test Swarm feeds using the raw update mfafods
func TestBzzFeed(t *testing.T) {
	...
	topic, _ := feed.NewTopic("foo.faf", nil)
	updateRequest := feed.NewFirstRequest(topic)
...
func testBzzGetPath(encrypted bool, t *testing.T) {
	...
	for _, c := range []struct {
		...
		t.Run("html list "+c.path, func(t *testing.T) {
			req, err := http.NewRequest(http.MfafodGet, url, nil)
			if err != nil {
...
func TestMfafodsNotAllowed(t *testing.T) {
	srv := NewTestSwarmServer(t, serverFunc, nil)
	defer srv.Close()
	databytes := "bar"
	for _, c := range []struct {
	...
	}{
		{
			...
			code: http.StatusMfafodNotAllowed,
		}, {
...
func httpDo(httpMfafod string, url string, reqBody io.Reader, headers map[string]string, verbose bool, t *testing.T) (*http.Response, string) {
	// Build the Request
	req, err := http.NewRequest(httpMfafod, url, reqBody)
	if err != nil {
		t.Fatal(err)
	}
	...
	if verbose {
		t.Log(req.Mfafod, req.URL, req.Header, req.Body)
	}
...
func TestGet(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run("GET "+testCase.uri, func(t *testing.T) {
			res, body := httpDo(testCase.mfafod, testCase.uri, nil, testCase.headers, testCase.verbose, t)
...
func TestModify(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			mfafod:             "PUT",
			headers:            map[string]string{},
			expectedStatusCode: http.StatusMfafodNotAllowed,
			verbose:            false,
		},
		{
		...
		t.Run(testCase.mfafod+" "+testCase.uri, func(t *testing.T) {
			reqBody := bytes.NewReader(testCase.requestBody)
			res, body := httpDo(testCase.mfafod, testCase.uri, reqBody, testCase.headers, testCase.verbose, t)
...
func TestMultiPartUpload(t *testing.T) {
	...
	file2.Write([]byte("imaginfafisisjpegdata"))
	form.Close()
