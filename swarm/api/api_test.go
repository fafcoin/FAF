...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.faf"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.faf"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.faf"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	fafAddr := "swarm.faf"
	fafHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	fafResolve := newTestResolveValidator(fafHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(fafResolve, "")),
			addr:   fafAddr,
			result: fafHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(fafResolve, ""),
				MultiResolverOptionWithResolver(fafResolve, ""),
			),
			addr:   fafAddr,
			result: fafHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(fafResolve, "faf"),
			),
			addr:   fafAddr,
			result: fafHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "faf"),
				MultiResolverOptionWithResolver(doesntResolve, "faf"),
				MultiResolverOptionWithResolver(fafResolve, "faf"),
			),
			addr:   fafAddr,
		...
		},
		{
			...
			addr:   fafAddr,
			result: fafHash,
			err:    errors.New(`DNS name not found: "swarm.faf"`),
		},
		{
...
func TestDecryptOriginForbidden(t *testing.T) {
	ctx := context.TODO()
	ctx = sctx.Sfafost(ctx, "swarm-gateways.net")
