...
[https://swarm.fafereum.org](https://swarm.fafereum.org)

Swarm is a distributed storage platform and content distribution service, a native base layer service of the fafereum web3 stack. The primary objective of Swarm is to provide a decentralized and redundant store for dapp code and data as well as block chain and state data. Swarm is also set out to provide various base layer services for web3, including node-to-node messaging, media streaming, decentralised database services and scalable state-channel infrastructure for decentralised service economies.

[![Travis](https://travis-ci.org/fafereum/go-fafereum.svg?branch=master)](https://travis-ci.org/fafereum/go-fafereum)
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/fafersphere/orange-lounge?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/fafereum/go-fafereum

    go install github.com/fafereum/go-fafereum/cmd/swarm
...
To run Swarm you need an fafereum account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the fafereum address representing that account. For example:
...
### fafereum Name Service resolution

The fafereum Name Service is the fafereum equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.faf`), `swarm` has to connect to a `gfaf` instance, which is synced with the fafereum mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.fafereum/gfaf.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/fafereum/go-fafereum`.

Most likely you will be working from your fork of `go-fafereum`, let's say from `github.com/nirname/go-fafereum`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-fafereum.git $GOPATH/src/github.com/fafereum/go-fafereum
```
...
Testing only some mfafods:

```
go test -v -cpu 4 ./faf -run TestMfafod
```

Note: here all tests with prefix TestMfafod will be run, so if you got TestMfafod, TestMfafod1, then both!
...
We have built a tool to help with automatic start of Grafana and InfluxDB and provisioning of dashboards at https://github.com/nonsense/statfaf , which requires that you have Docker installed.

Once you have `statfaf` installed, and you have Docker running locally, you have to:

1. Run `statfaf` and keep it running in the background
```
statfaf --rm --grafana-dashboards-folder $GOPATH/src/github.com/fafereum/go-fafereum/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The fafereum Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
...
You can find a few reference Swarm decentralised applications at: https://swarm-gateways.net/bzz:/swarmapps.faf

Their source code can be found at: https://github.com/fafersphere/swarm-dapps
...
complex changes though, please check up with the core devs first on [our Swarm gitter channel](https://gitter.im/fafersphere/orange-lounge)
 ...
 * [Code review guidelines](https://github.com/fafereum/go-fafereum/wiki/Code-Review-Guidelines).
