...
func (w *wizard) makeGenesis() {
	...
	//fmt.Println(" 1. fafash - proof-of-work")
	//fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of fafash, we're pretty much done
		genesis.Config.fafash = new(params.fafashConfig)
...
// importGenesis imports a Gfaf genesis spec into puppfaf.
...
func (w *wizard) manageGenesis() {
	// Figure out whfafer to modify or export the genesis
	//fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alfaf.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppfaf and Gfaf
		gfafJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gfafJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gfafJson)

		// Export the genesis spec used by Alfaf (formerly C++ fafereum)
		if spec, err := newAlfafGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Alfaf chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alfaf", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly fafereumJ
