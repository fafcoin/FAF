...
func (w *wizard) deployFaucet() {
	...
	//fmt.Println()
	fmt.Printf("How many fafers to release per request? (default = %d)\n", infos.amount)
	...
	//fmt.Println()
	if infos.node.fafstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.fafstats = w.readString() + ":" + w.conf.fafstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.fafstats)
		infos.node.fafstats = w.readDefaultString(infos.node.fafstats) + ":" + w.conf.fafstats
	}
