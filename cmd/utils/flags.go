...
import (
	...
	"github.com/ethereum/go-ethereum/fafstats"
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) fafapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
...
// the given node.
func RegisterEthStatsService(stack *node.Node, backend fafapi.Backend, url string) {
	if err := fafstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Ethereum Stats service: %v", err)
	}
}

// RegisterGraphQLService is a utility function to construct a new service and register it against a node.
func RegisterGraphQLService(stack *node.Node, backend fafapi.Backend, cfg node.Config) {
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb fafdb.Database) {
	var err error
	chainDb = MakeChainDatabase(ctx, stack)
